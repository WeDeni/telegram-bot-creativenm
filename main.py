...
from database import bot, bot_name, month_mapping, event_map
import admin_cmds
import menu
...
def call_reg(call):
    ...
    if not row: # Если запись не найдена
        tg_name_1 = f'{call.from_user.first_name} ' if call.from_user.first_name else ''
        tg_name_2 = f'{call.from_user.last_name} ' if call.from_user.last_name else ''
        tg_nickname = f'@{call.from_user.username}'
        full_username = f'{tg_name_1}{tg_name_2}{tg_nickname}'
        db.add_user_name(call.from_user.id, full_username)
    ...
    if key == None: # Если нажали просто /start
        ...
        for event_id in event_map:
            if db.get_reg_info(event_id, msg.from_user.id):
                ...
                time = event_id[15:].replace('-', ':')
                event_date = f'*{day} {month}* в *{time}*'
                notification_mode = row[5]
                if notification_mode == 1: icon = '🔔'
                elif notification_mode == 2: icon = '🔕'
                else: icon = '🚫'
                user_event_list = user_event_list + f'*\n\n{event}*\n📅 {event_date} ({icon})\n🎟 Билетиков: *{tickets}* ([Изменить](https://t.me/{bot_name}?start={event_id}))'
        if user_event_list == '⬇️ Список мероприятий, куда вы записаны.':
            user_event_list = 'Вы никуда не записаны 🤷‍♂️\nСледите за новостями в наших соц сетях:\n[Волонтеры Культуры в VK](https://vk.com/volonterycultury_nao)\n[Креатив в Нарьяне в Телеграм](https://t.me/Creative_NM)'
